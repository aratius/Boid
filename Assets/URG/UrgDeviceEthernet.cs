...
//		messageQueue = Queue.Synchronized(new Queue());
		...
		// 	IAsyncResult result = tcp.BeginConnect(ip_address, port_number, null, null);
			
		// 	// タイムアウト処理
		// 	bool success = result.AsyncWaitHandle.WaitOne(TimeSpan.FromSeconds(2));
		// 	if (!success) {
		// 		throw new Exception("Failed to urg connect. IP = " + ip_address.ToString() + ", Port = " + port_number.ToString());
		// 	}
